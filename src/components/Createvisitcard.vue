<template>
    <div class="content">
        ...
        <van-cell>
            请选择就诊卡性别：
            <van-radio-group v-model="radio" @change="getGender">
        ...
        <!-- <van-cell-group class="aggregate"> -->
            ...
            <!-- <van-field maxlength="18" left-icon="coupon-o" class="Certificates_number" v-model="cnumber" clearable placeholder="请输入身份证号码" />
        </van-cell-group> -->
        <van-cell-group>
           <van-field v-model="cnumber" clearable left-icon="coupon-o" placeholder="请输入身份证号码" />
        </van-cell-group>
        <van-cell-group>
...
import { addPatientCard } from '../api/api'
...
export default {
    ...
    methods:{
        ...
        suBmission:function(){//确认添加
            let data = new FormData()
            data.append('patientName',this.value)//患者姓名
            data.append('sex',this.radio)//性别，0：男，1：女，2：未知
            data.append('idCardNo',this.cnumber)//身份证号
            data.append('phone',this.phone)//手机号
            console.log('参数信息:',this.value+this.radio+this.cnumber+this.phone)
            if(this.value == '' || this.cnumber == '' || this.phone == ''){
                alert('请将信息填写完整！')
                return
            }else{
                postPatientCard(data).then((res) => {
                    if(res.code == 1){
                        // alert(res.data)
                        this.$router.push({path:'/smartanswer'})
                    }else{
                        console.log(res)
                    }
                })
